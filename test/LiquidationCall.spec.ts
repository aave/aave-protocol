...
contract("LendingPool liquidation", async ([deployer, ...users]) => {
  ...
  it("LIQUIDATION - Deposits ETH, borrows DAI/Check liquidation fails because health factor is above 1", async () => {
    ...
    const aEthInstance: ATokenInstance = await getTruffleContractInstance(
      artifacts,
      ContractId.AToken,
      await _lendingPoolCoreInstance.getReserveATokenAddress(_ethereumAddress),
    )
    ...
    const amountETHtoDeposit = await convertToCurrencyDecimals(_ethereumAddress, "1")
    ...
    await _lendingPoolInstance.borrow(_daiAddress, amountDAIToBorrow, RATEMODE_FIXED, "0", {
      from: _borrowerAddress,
    })
  ...
  it("LIQUIDATION - Drop the health factor below 1", async () => {
    ...
    await _priceOracleInstance.setAssetPrice(
      _daiAddress,
      new BigNumber(daiPrice).multipliedBy(1.3).toFixed(0),
    )
  ...
  it("LIQUIDATION - Liquidates the borrow", async () => {
    ...
    expect(daiReserveDataAfter.availableLiquidity).to.be.bignumber.equal(
      new BigNumber(daiReserveDataAfter.availableLiquidity).plus(amountToLiquidate).toFixed(0),
      "Invalid health factor",
    )
    ...
    expect(liquidatorReserveData.currentATokenBalance).to.be.bignumber.gt(
      "0",
      "Invalid liquidator aETH balance after liquidation",
    )
  })
})
