...
contract("LendingPool - token economy tests", async ([deployer, ...users]) => {
  ...
  it("BORROW - Test user cannot borrow using the same currency as collateral", async () => {
    ...
    //user 2 deposits 1000 DAI, tries to borrow. Needs to be reverted as you can't borrow at a fixed rate with the same collateral as the currency.
    ...
    //user 2 tries to borrow
    await expectRevert(
      _lendingPoolInstance.borrow(_daiAddress, amountDAIToBorrow, RATEMODE_FIXED, "0", {
        from: _borrowerAddress,
      }),
      "User is trying to borrow an amount that is smaller than what he deposited in the same currency, which may lead to abuses",
    )
  })
  ...
  it("BORROW - Test user cannot borrow  more than 25% of the liquidity available", async () => {
    ...
    const amountADAIToRedeem = await convertToCurrencyDecimals(aDaiInstance.address, "100000")
    ...
    const amountETHToDeposit = await convertToCurrencyDecimals(_ethereumAddress, "5")
    await _lendingPoolInstance.deposit(_ethereumAddress, amountETHToDeposit, "0", {
      from: _borrowerAddress,
    ...
    await expectRevert(
      ...
      "User is trying to borrow too much liquidity at a fixed rate",
    )
  })
})
